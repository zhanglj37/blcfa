	...
	## category & point: used for category data (under development)
{
	...
	### source("read_observed.R")
	### source("Gibbs.R")
	cat("E-step: See 'log.txt' for details.  \n")
	                      ...
	                      .export = c("IDY_matrix_fun", "set_int_fun", "read_dataset", "gibbs_fun")) %is_par%
	{
		...
		chainlist <- gibbse_fun(MCMAX,NZ,NY,N,Y,LY_int,IDMU,IDMUA,IDY,
	...
	sigpsx_list <- sig_psx_fun(NZ,NY,dataset,resultlist,hpdlist,interval_psx,'e')
	...
	if (convergence)
	{
		cat("C-step: See 'log.txt' for details.  \n")
							  ...
							  .export = c("IDY_matrix_fun", "set_int_fun", "read_dataset", "gibbs_fun")) %is_par%
		{
			...
			IDY<-IDY_matrix_pecfa_fun(dataset,mmvar,mmvar_loc,hpdlist,NY,NZ)
				#vector indicating whether MU should be added to each measurement equation or not.
				#Added(1), remove(0).
			IDMU<-rep(1,NY)  # now MU is estimated
			IDMUA<-any(as.logical(IDMU))
		...
		if (convergence)
		{
			...
			cat('Error: The convergence criterion is not satisfied in C-step.  \n')
			...
			savePlot(filename = "EPSR_c",
					type = "png",
					device = dev.cur(),
					restoreConsole = TRUE)
		}
	}else{
		cat('Error: The convergence criterion is not satisfied in E-step.  \n')
		cat('Please refer to the epsr graph and increase the value of N.burn and MCMAX.')
