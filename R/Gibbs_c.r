...
gibbsc_fun<-function(MCMAX,NZ,NY,N,Y,LY_int,IDMU,IDMUA,IDY,nthin,mmvar,
...
for(g in 1:MCMAX){
	...
	#apost1<-a_lamsq+NLY1;
	#stau<-sum(tausq)
	apost1<-a_lamsq+rowSums(IDY==-1);
	stau<-rowSums(tausq)

	#sample lambda
	bpost1<-b_lamsq + stau/2  # C is the presicion matrix
	lamsq<-rep(0,lj)
	for(i in 1:lj){lamsq[i]<- rgamma(1, shape=apost1[i], rate=bpost1[i])}
	...
	for(j in 1:NY){
	  ...
	  if(len>0){
		...
		#invconvar<-1/convar
		  ...
		  lind=sum(Lj[1:j])
		  mu_p1<-pmin(sqrt(lamsq[lind]/Cadj1), 10^12)
